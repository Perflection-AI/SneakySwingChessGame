...
| 卡牌 | 效果 | 一句话 |
| ---- | ---- | ---- |
| Left Wind | 所有球左偏 5–15yd | 风吹啊吹 |
| Right Wind | 所有球右偏 5–15yd | 反向的风 |
| Rain | 推进距离 −30% | 下雨了，打不远 |
| Tailwind | 推进距离 +20% | 顺风！冲冲冲 |
| Fog | 所有球随机偏移 3–8yd | 雾太大看不清了 |
...
| 卡牌 | 效果 | 目标 | 一句话 |
| ---- | ---- | ---- | ---- |
| Squirrel Tax | 球被随机位移 5–20yd | 对手 | 松鼠不讲理 |
| Mole Tunnel | 球往前跳 10–30yd | 自己 | 地鼠帮忙 |
| Angry Goose | 对手下一杆 Touch −2 | 对手 | 鹅在看着你 |
| Sly Fox | 对手下一杆 Aim −2 | 对手 | 狐狸偷走了方向感 |
| Lucky Rabbit | 球往前跳 5–15yd | 自己 | 兔子带你走捷径 |
...
`appConfig.cards.enabled` 控制开关。关闭时不发牌、不显示 UI、直接自动击球。

数据结构：`src/cards.js`（卡牌定义）、`useGameReducer.js`（state.hand[] / activeCard / activeWeather / cardPenalties）、`CardPicker.jsx`（UI）、`Board.jsx` ActiveEffects（效果面板）。
...
```text
上传训练 → 生成 Power/Aim/Touch/Issue
→ 18 洞比赛（默认 Par 4 开局）
→ 每洞：丢弃旧手牌 → 发 1 张卡（上限 4 张）
→ 每个 Stage 完成后：发牌（hand < 4 时）
→ card_picking → USE/SKIP/EXCHANGE → 效果生效
...
Debug 面板（game mode）提供单一 Map ON/OFF 按钮。开启时自动扫描 `public/map/map_1/` 到 `map_20/`：
